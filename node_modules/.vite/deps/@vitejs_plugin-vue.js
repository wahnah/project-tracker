...
var import_tty2 = __toESM(require_tty(), 1);
...
function createDescriptor(filename, source, { root, isProduction, sourceMap, compiler, template }, hmr = false) {
  const { descriptor, errors } = compiler.parse(source, {
    filename,
    sourceMap,
    templateParseOptions: template == null ? void 0 : template.compilerOptions
  });
  const normalizedPath = slash(import_node_path2.default.normalize(import_node_path2.default.relative(root, filename)));
...
function getDescriptor(filename, options, createIfNotFound = true, hmr = false, code) {
  ...
  if (createIfNotFound) {
    const { descriptor, errors } = createDescriptor(
      filename,
      code ?? import_node_fs2.default.readFileSync(filename, "utf-8"),
      options,
      hmr
    );
...
function getHash(text) {
  return (0, import_node_crypto2.createHash)("sha256").update(text).digest("hex").substring(0, 8);
}
...
function resolveTemplateCompilerOptions(descriptor, options, ssr) {
  ...
  if (transformAssetUrls === false)
    ...
    if (filename.startsWith(options.root)) {
      ...
      assetUrlOptions = {
        base: (((_b = options.devServer.config.server) == null ? void 0 : _b.origin) ?? "") + devBase + slash(import_node_path2.default.relative(options.root, import_node_path2.default.dirname(filename))),
  ...
  return {
    ...options.template,
    id,
    ast: canReuseAST(options.compiler.version) ? (_h = descriptor.template) == null ? void 0 : _h.ast : void 0,
    filename,
    scoped: hasScoped,
...
function resolveScript(descriptor, options, ssr, customElement) {
  ...
  if (!options.isProduction && (resolved == null ? void 0 : resolved.deps)) {
...
function requireNode() {
  ...
  hasRequiredNode = 1;
  (function(module, exports) {
    const tty = import_tty2.default;
    const util = import_util2.default;
    exports.init = init;
    exports.log = log;
    exports.formatArgs = formatArgs;
    exports.save = save;
...
function hasScriptChanged(prev, next) {
  ...
  if (!isEqualBlock(prev.script, next.script) && !isEqualAst(prevScript == null ? void 0 : prevScript.scriptAst, nextScript == null ? void 0 : nextScript.scriptAst)) {
    return true;
  }
  if (!isEqualBlock(prev.scriptSetup, next.scriptSetup) && !isEqualAst(prevScript == null ? void 0 : prevScript.scriptSetupAst, nextScript == null ? void 0 : nextScript.scriptSetupAst)) {
    return true;
  }
...
async function transformMain(code, filename, options, pluginContext, ssr, customElement) {
  ...
  if (import_node_fs2.default.existsSync(filename)) {
    getDescriptor(
      filename,
      options,
      true,
  ...
  if (devToolsEnabled || devServer && !isProduction) {
    attachedProps.push([
      `__file`,
      JSON.stringify(isProduction ? import_node_path2.default.basename(filename) : filename)
    ]);
  }
  ...
  if (ssr) {
    const normalizedFilename = normalizePath$3(
      import_node_path2.default.relative(options.root, filename)
    );
    output.push(
...
async function genScriptCode(descriptor, options, pluginContext, ssr, customElement) {
  ...
  if (script) {
    if (canInlineMain(descriptor, options)) {
      ...
      const langFallback = script.src && import_node_path2.default.extname(src2).slice(1) || "js";
  ...
  return {
    ...
    config(config) {
      ...
      return {
        ...
        define: {
          ...
          __VUE_PROD_HYDRATION_MISMATCH_DETAILS__: ((_d = config.define) == null ? void 0 : _d.__VUE_PROD_HYDRATION_MISMATCH_DETAILS__) ?? false
        },
        ssr: {
    ...
    buildStart() {
      ...
      if (compiler.invalidateTypeCache) {
        (_a = options.value.devServer) == null ? void 0 : _a.watcher.on("unlink", (file) => {
    ...
    load(id, opt) {
      ...
      if (query.vue) {
        if (query.src) {
          return import_node_fs2.default.readFileSync(filename, "utf-8");
        }
